...
class TwoLayerNet():
    ...
    def __init__(self, X, input_size, hidden_size, output_size, std=1e-4):
         ...
         self.params = {}
         np.random.normal(0,math.sqrt(1/X.shape[0]),(hidden_size, output_size))
#         self.params["W1"] = std * np.random.randn(input_size, hidden_size)
         self.params["W1"] = std * np.random.normal(0,math.sqrt(1/X.shape[0]),(input_size, hidden_size))
         self.params["b1"] = np.random.randn(hidden_size) #np.zeros(hidden_size)
#         self.params["W2"] = std * np.random.randn(hidden_size, output_size)
         self.params["W2"] = std * np.random.normal(0,math.sqrt(1/X.shape[0]),(hidden_size, output_size))
         self.params["b2"] = np.random.randn(output_size) #np.zeros(output_size)
    ...
    def forward(self, X, y=None):
        ...
        h = np.dot(X, W1)+b1
        relu = np.zeros((h.shape[0],h.shape[1]))
        a = np.maximum(h,relu)
        scores = None
        o = np.dot(a,W2)+b2
        ...
        LL=0
        p_ones = np.ones((p.shape[0],p.shape[1]))-p
